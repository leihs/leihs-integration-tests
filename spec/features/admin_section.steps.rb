...
# Override: navigate directly with a pre-built filter_q so all pool items are
...
step "I add a search filter" do
  ...
  filter_q = CGI.escape(EMPTY_SEARCH_EDIT_FILTER_Q)
  visit "/inventory/#{pool_id}/search-edit?filter_q=#{filter_q}&page=1&size=50"
...
def fill_test_filter_value(index, label)
  ...
  when "test_checkbox"
    ...
    scroll_and_click(find(:xpath, checkbox_xpath, visible: :all, wait: 10))
  end
end
