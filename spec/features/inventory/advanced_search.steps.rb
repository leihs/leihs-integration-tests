def search_edit_filter_d_in_url?
  query = URI.parse(current_url).query.to_s
  Rack::Utils.parse_nested_query(query).key?("filter_d")
end
...
def click_search_edit_add_filter_prompt
  ...
  # filter_d is written later by the 300ms debounced form watch — not on this click.
...
step "I see :count search result items" do |count|
  expect(search_edit_filter_d_in_url?).to be(true)
