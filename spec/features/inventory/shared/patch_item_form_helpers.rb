...
def fill_scan_edit_select(index, option_label)
  ...
  within("[data-test-id='#{patch_value_name(index)}-options']") do
    label_pattern =
      if option_label.is_a?(Regexp)
        option_label
      else
        /#{Regexp.escape(option_label)}/i
      end
    find(:button, text: label_pattern, match: :first).click
  end
  dismiss_patch_field_dropdown
end
...
def set_all_built_in_patch_fields
  ...
  fill_scan_edit_select(idx, /Yes|Ja/i)
  ...
  expect(page).to have_content(values[:room_name], wait: 5)
  idx += 1
